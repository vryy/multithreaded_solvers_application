...
//****************************************************************************80
void MeshRCM::Renumber(ModelPart& r_model_part)
{
    // extract the element connectivities
    ModelPart::ElementsContainerType& pElements = r_model_part.Elements();
    int node_num = r_model_part.NumberOfNodes();
    std::vector<std::vector<int> > element_nodes;
    std::vector<int> element_type;
    for (typename ModelPart::ElementsContainerType::ptr_iterator it = pElements.ptr_begin(); it != pElements.ptr_end(); ++it)
    {
        ...
        else if(name.find("Quadrilateral2D8") != std::string::npos) type = 4;
        else if(name.find("Tetrahedra3D4") != std::string::npos)    type = 5;
        else if(name.find("Tetrahedra3D10") != std::string::npos)   type = 6;
        else if(name.find("Hexahedra3D8") != std::string::npos)     type = 7;
        else if(name.find("Hexahedra3D20") != std::string::npos)    type = 8;
        else {}
    ...
    //
    mesh_adj_count ( node_num, element_nodes.size(), element_nodes, element_type, &adj_num, adj_row );
    ...
    //
    double bandwidth = adj_bandwidth ( node_num, adj_num, adj_row, adj );
    std::cout << "  ADJ bandwidth = " << bandwidth << "\n";

    //
    //  GENRCM computes the RCM permutation.
    //
    int* perm = genrcm ( node_num, adj_num, adj_row, adj );
    //
    //  Compute the inverse permutation.
    //
    int* perm_inv = perm_inverse3 ( node_num, perm );

    //
    //  Compute the bandwidth of the permuted array.
    //
    bandwidth = adj_perm_bandwidth ( node_num, adj_num, adj_row, adj, perm, perm_inv );
    std::cout << "  ADJ bandwidth after RCM permutation = " << bandwidth << "\n";

    //
    // Renumber the nodes
    //
    ModelPart::NodesContainerType& pNodes = r_model_part.Nodes();
    int cnt = 0;
    for (typename ModelPart::NodesContainerType::ptr_iterator it = pNodes.ptr_begin(); it != pNodes.ptr_end(); ++it)
//        (*it)->SetId(perm[cnt++]+1);
        (*it)->SetId(perm_inv[cnt++]+1); //TODO: check why it created good matrix sparsity pattern
    ...
    //  Terminate.
    //
    std::cout << "MESH_RCM completed\n";
//    timestamp ( );
}

//****************************************************************************80

int MeshRCM::adj_bandwidth ( int node_num, int adj_num, int adj_row[], int adj[] )
...
int* MeshRCM::genrcm ( int node_num, int adj_num, int adj_row[], int adj[] )
...
int MeshRCM::i4_max ( int i1, int i2 )
...
int MeshRCM::i4_min ( int i1, int i2 )
...
void MeshRCM::i4_swap ( int *i, int *j )
...
int MeshRCM::i4col_compare ( int m, int n, int a[], int i, int j )
...
void MeshRCM::i4col_sort_a ( int m, int n, int a[] )
...
void MeshRCM::i4col_sort2_a ( int m, int n, int a[] )
...
int MeshRCM::i4col_sorted_unique_count ( int m, int n, int a[] )
...
void MeshRCM::i4col_swap ( int m, int n, int a[], int icol1, int icol2 )
...
void MeshRCM::i4vec_reverse ( int n, int a[] )
...
bool MeshRCM::perm_check ( int n, int p[], int base )
...
int* MeshRCM::perm_inverse3 ( int n, int perm[] )
...
void MeshRCM::r8col_permute ( int m, int n, int p[], int base, double a[] )
...
void MeshRCM::sort_heap_external ( int n, int *indx, int *i, int *j, int isgn )
...
//  Purpose:
//
//    MESH_ORDER4_ADJ_COUNT counts the number of nodal adjacencies.
//
//  Discussion:
//
//    Assuming that the mesh is to be used in a finite element
...
//
  pair_unique_num = i4col_sorted_unique_count ( 2, pair_num, &pair[0] );
//
...
//  Purpose:
//
//    MESH_ORDER4_ADJ_SET sets the nodal adjacency matrix.
//
//  Discussion:
//
...
//
{
  ...
  int e;
//
//  Create pair of edges from element.
//
